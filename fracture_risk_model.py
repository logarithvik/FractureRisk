...
and predict osteoporotic fracture risk from clinical features.

Modes:
 1. `prepare`   - Load SAS files, engineer features, save as CSV.
 2. `train`     - Train model on CSV dataset with class_weight and Dropout.
 3. `predict`   - Run inference using weight/height inputs to calculate BMI.
...
NUMERICAL_FEATURES = ["Age", "BMI", "Smoking"]
TARGET = "Fracture"
...
def prepare(sas_dir: str, out_csv: str):
    """
    Load NHANES XPT files, engineer Age, Sex, BMI, Smoking, Fracture,
    and save consolidated CSV.
    """
    ...
    final = data[['SEQN', 'Age', 'Sex', 'BMI', 'Smoking', 'Fracture']]
...
def train(csv_path: str, out_dir: str, epochs: int=50, batch_size: int=32):
    ...
    model_path = artefacts_dir / "model.keras"
    prep_path = artefacts_dir / "preprocessor.pkl"
...
# ----------------------------------------------------------------------
# Inference with weight & height for BMI
# ----------------------------------------------------------------------
def load_artefacts(model_path: str, prep_path: str):
    return load_model(model_path), joblib.load(prep_path)

def predict_fracture_risk(age: float, weight: float, height: float, sex: str, smoking: int,
                           model=None, preprocessor=None, model_path=None, prep_path=None) -> float:
    """Compute BMI from weight/height and return fracture probability."""
    ...
    df = pd.DataFrame([[age, bmi, sex, smoking]], columns=['Age','BMI','Sex','Smoking'])
...
# ----------------------------------------------------------------------
def parse_args():
    parser = argparse.ArgumentParser(description="Fracture risk utility")
    ...
    tr_p = sub.add_parser("train", help="Train model")
    ...
    pd_p = sub.add_parser("predict", help="Predict fracture risk")
...
def main():
    ...
    if args.mode == 'prepare':
        ...
        prob = predict_fracture_risk(
            ...
            sex=args.sex, smoking=args.smoking,
            model_path=args.model, prep_path=args.prep
        )
        print(f"Predicted fracture probability: {prob:.3f}")
    else:
